...
type Room={id:string;room_number:string;monthly_rent:number;deposit_amount:number;status:string};
...
export default function TenantsPage(){
  ...
  async function load(){
    ...
    const [t,r,c]=await Promise.all([
      ...
      workspace.supabase.from('rooms').select('id,room_number,monthly_rent,deposit_amount,status').eq('property_id',pid).is('archived_at',null).order('room_number'),
    ...
    setRooms((r.data||[]) as Room[]); setContracts((c.data||[]) as Contract[]);
  }
  ...
  const filtered=tenants.filter(t=>`${t.full_name} ${t.phone||''} ${t.email||''}`.toLowerCase().includes(search.toLowerCase()));
  const availableRooms=rooms.filter(r=>r.status==='vacant'||r.id===form.room_id);
  return <AppShell>
    ...
    <div className="toolbar"><div className="search-box"><Icon name="search" size={17}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ค้นหาชื่อ เบอร์โทร หรืออีเมล..."/></div></div>
