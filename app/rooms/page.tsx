...
export default function RoomsPage(){
  ...
  return <AppShell>
    <PageTitle title="จัดการห้องพัก" subtitle={workspace.activeProperty?`โครงการ ${workspace.activeProperty.name}`:'สร้างโครงการก่อนเพิ่มห้อง'} action={workspace.can('rooms.create')?<button className="primary-btn" onClick={createNew} disabled={!workspace.activePropertyId}><Icon name="plus" size={18}/> เพิ่มห้อง</button>:undefined}/>
    ...
    <div className="rooms-grid">{filtered.map((r,i)=><article className="room-card" key={r.id}>
      ...
      <div className="room-body"><div><div><span className="room-kicker">ROOM</span><h3>ห้อง {r.room_number}</h3></div><strong>฿ {Number(r.monthly_rent).toLocaleString()} <small>/ เดือน</small></strong></div><p>{buildings.find(b=>b.id===r.building_id)?.name||'ยังไม่ระบุอาคาร'}</p><div className="room-meta"><span>ชั้น {r.floor_number||'-'}</span><span>{r.size_sqm||'-'} ตร.ม.</span><span>มัดจำ ฿{Number(r.deposit_amount).toLocaleString()}</span></div><CrudMenu canEdit={workspace.can('rooms.update')} canDelete={workspace.can('rooms.delete')} onEdit={()=>editRoom(r)} onDelete={()=>void remove(r)}/></div>
    </article>)}</div>}
