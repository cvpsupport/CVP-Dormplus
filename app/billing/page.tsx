...
export default function BillingPage(){
  ...
  async function load(){
    ...
    const [i,c,t,r]=await Promise.all([
      ...
      workspace.supabase.from('rooms').select('id,room_number').eq('property_id',pid).is('archived_at',null)
    ]);
    if(i.error)setToast({message:i.error.message,tone:'error'});else setInvoices((i.data||[]) as Invoice[]);setContracts((c.data||[]) as Contract[]);setTenants((t.data||[]) as Tenant[]);setRooms((r.data||[]) as Room[]);
  }
  ...
  function newInvoice(){setEditing(null);const c=contracts[0];setForm({...emptyForm,contract_id:c?.id||'',total:c?String(c.rent_amount):'',due_date:new Date(Date.now()+7*86400000).toISOString().slice(0,10)});setOpen(true);}
  ...
  const totalPaid=invoices.filter(i=>i.status==='paid').reduce((s,i)=>s+Number(i.total),0);const pending=invoices.filter(i=>['issued','partially_paid'].includes(i.status)).reduce((s,i)=>s+Number(i.total),0);const overdue=invoices.filter(i=>i.status==='overdue').reduce((s,i)=>s+Number(i.total),0);const collection=invoices.length?Math.round(invoices.filter(i=>i.status==='paid').length/invoices.length*100):0;
  const filtered=invoices.filter(i=>i.invoice_number.toLowerCase().includes(search.toLowerCase()));
  return <AppShell>
    <PageTitle title="การเงินและการชำระ" subtitle="สร้าง แก้ไข และลบใบแจ้งหนี้จาก Supabase" action={<button className="primary-btn" onClick={newInvoice} disabled={!contracts.length}><Icon name="plus" size={18}/> สร้างใบแจ้งหนี้</button>}/>
    <section className="stats-grid compact"><StatCard icon="finance" label="ชำระแล้ว" value={`฿${totalPaid.toLocaleString()}`} note={`${invoices.filter(i=>i.status==='paid').length} รายการ`}/><StatCard icon="contract" label="รอชำระ" value={`฿${pending.toLocaleString()}`} note="ยอดค้างปัจจุบัน" tone="amber"/><StatCard icon="bell" label="เกินกำหนด" value={`฿${overdue.toLocaleString()}`} note={`${invoices.filter(i=>i.status==='overdue').length} รายการ`} tone="red"/><StatCard icon="chart" label="อัตราเก็บเงิน" value={`${collection}%`} note="จากจำนวนใบแจ้งหนี้" tone="blue"/></section>
    <div className="toolbar"><div className="search-box"><Icon name="search" size={17}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ค้นหาเลขที่บิล..."/></div></div>
    {!workspace.activePropertyId?<EmptyState title="ยังไม่มีโครงการ" description="สร้างโครงการก่อนใช้งานการเงิน"/>:!contracts.length&&!invoices.length?<EmptyState title="ยังไม่มีสัญญาเช่า" description="เพิ่มผู้เช่าและเลือกห้องก่อน ระบบจึงจะสร้างสัญญาและใบแจ้งหนี้ได้"/>:!filtered.length?<EmptyState title="ยังไม่มีใบแจ้งหนี้" description="กดสร้างใบแจ้งหนี้เพื่อเริ่มต้น" action={<button className="primary-btn" onClick={newInvoice}><Icon name="plus" size={17}/> สร้างใบแจ้งหนี้</button>}/>:<section className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>เลขที่</th><th>ห้อง</th><th>ผู้เช่า</th><th>ยอดรวม</th><th>ครบกำหนด</th><th>สถานะ</th><th>จัดการ</th></tr></thead><tbody>{filtered.map(i=>{const c=contractMap.get(i.contract_id);return <tr key={i.id}><td><b>{i.invoice_number}</b></td><td>{c?roomMap.get(c.room_id)||'-':'-'}</td><td>{c?tenantMap.get(c.tenant_id)||'-':'-'}</td><td><b>฿{Number(i.total).toLocaleString()}</b></td><td>{i.due_date}</td><td><Badge tone={statusTone[i.status]||'gray'}>{statusLabel[i.status]||i.status}</Badge></td><td><CrudMenu onEdit={()=>editInvoice(i)} onDelete={()=>void remove(i)}/></td></tr>})}</tbody></table></div></section>}

    <Modal open={open} title={editing?'แก้ไขใบแจ้งหนี้':'สร้างใบแจ้งหนี้'} onClose={()=>setOpen(false)}><form className="crud-form" onSubmit={submit}><div className="form-grid"><label className="full">สัญญา<select required value={form.contract_id} onChange={e=>chooseContract(e.target.value)} disabled={!!editing}><option value="">เลือกสัญญา</option>{contracts.map(c=><option key={c.id} value={c.id}>ห้อง {roomMap.get(c.room_id)||'-'} · {tenantMap.get(c.tenant_id)||'-'} · {c.contract_number}</option>)}</select></label><label>รอบบิล<input type="date" required value={form.billing_period} onChange={e=>setForm({...form,billing_period:e.target.value})}/></label><label>ครบกำหนด<input type="date" required value={form.due_date} onChange={e=>setForm({...form,due_date:e.target.value})}/></label><label>ยอดรวม<input type="number" min="0" required value={form.total} onChange={e=>setForm({...form,total:e.target.value})}/></label><label>สถานะ<select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option value="draft">ร่าง</option><option value="issued">รอชำระ</option><option value="partially_paid">ชำระบางส่วน</option><option value="paid">ชำระแล้ว</option><option value="overdue">เกินกำหนด</option><option value="cancelled">ยกเลิก</option></select></label></div><FormActions saving={saving} onCancel={()=>setOpen(false)}/></form></Modal><Toast message={toast?.message||null} tone={toast?.tone} onClose={()=>setToast(null)}/>
  </AppShell>;
}
