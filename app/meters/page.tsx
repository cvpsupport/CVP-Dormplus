...
export default function MetersPage(){
  ...
  const [period,setPeriod]=useState(monthNow()); const [search,setSearch]=useState(''); const [open,setOpen]=useState(false); const [form,setForm]=useState<ReadingForm>(emptyForm); const [saving,setSaving]=useState(false); const [toast,setToast]=useState<{message:string;tone:'success'|'error'}|null>(null);
  ...
  const visible=rooms.filter(r=>r.room_number.toLowerCase().includes(search.toLowerCase())||(r.building_id?buildingMap.get(r.building_id)||'':'').toLowerCase().includes(search.toLowerCase()));
  const waterUsage=periodReadings.filter(r=>meters.find(m=>m.id===r.meter_id)?.meter_type==='water').reduce((s,r)=>s+n(r.usage),0); const electricUsage=periodReadings.filter(r=>meters.find(m=>m.id===r.meter_id)?.meter_type==='electricity').reduce((s,r)=>s+n(r.usage),0); const totalAmount=periodReadings.reduce((s,r)=>s+n(r.amount),0); const completed=new Set(periodReadings.map(r=>r.room_id)).size;
